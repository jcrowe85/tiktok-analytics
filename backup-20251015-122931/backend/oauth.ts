 ...
 */
export async function refreshAccessToken(refreshToken: string, userId?: number): Promise<TikTokAuthTokens> {
    ...
    try {
      ...
      await executeQuery(
        `UPDATE users SET 
         ...
         tiktok_token_expires_at = NOW() + INTERVAL '24 hours'
         WHERE id = $3`,
        [tokens.access_token, tokens.refresh_token, userId]
      );
      console.log(`✅ Updated tokens for user ${userId} in database`);
    } catch (error) {
      console.error('❌ Failed to update tokens in database:', error);
      // Fallback to .env update for backward compatibility
      updateEnvFile(tokens.access_token, tokens.refresh_token);
    }
  } else {
    // Fallback to .env update if no userId provided
    updateEnvFile(tokens.access_token, tokens.refresh_token);
  }
